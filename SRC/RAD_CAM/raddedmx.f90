...
!
   ...
   real(r4) gama                ! Term in direct reflect and transmissivity
   real(r4) el                   ! Term in alpha,gama,n,u
...
!
   ...
   real(r4) gam                  ! Temporary for gama
...
!
   ...
   gama(w,uu,g,e) = .50_r4*w*((3._r4*g*(1._r4-w)*uu*uu + 1._r4)/(1._r4-e*e*uu*uu))
...
!
   do k=0,pver
      do i=1,ndayc
            ...
            gs   = asys(gtot,ftot)
            lm   = el(ws,gs)
  ! Marat: Avoid division by 0 in alpha and gama functions in rare cases:
            cosz=coszrs(i)
            if(abs(1.-cosz*cosz*lm*lm).lt.1.e-5) cosz=min(1.,sqrt((1.-1.e-5)/(lm*lm)))
            alp  = alpha(ws,cosz,gs,lm)
            gam  = gama(ws,cosz,gs,lm)
            ue   = u(ws,gs,lm)
!
